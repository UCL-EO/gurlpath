...
class URL(urlpath.URL,urllib.parse._NetlocResultMixinStr, PurePath):
    ...
    def __init__(self,*args,**kwargs):
        ...
        # back to tuple
        args = tuple(args)
        # 401 flag init to False
        self.fourOhOne = False

    def init(self,**kwargs):
        '''
        Initialise URL object with items
        from kwargs and set up database.
        
        First:
            any items in kwargs are passed 
            through to this object, 
            e.g. kwargs: {xx=3} -> self.xx = 3
        Then:
            we test to see if self.database exists
            and is of type Database
            If not, then set up database from
            self.db_file
        
        '''
        # ginit is a list function
        self.__dict__.update(ginit(self,**kwargs))
        if 'database' in self.__dict__ and \
            type(self.database) == Database:
            # already have database stored
            pass
        else:
